	...
	public function insert($params)
	{
		...
		if ($this->validateInsert($params)) {
			...
			if (isset($params['imageUri'])) {
				/*echo "Tiene imagen";
				exit;*/
				$img_array = explode(';base64,', $params['imageUri']); //datos de la imagen
				...
				$params['imageUri'] = $nombre_imagen . '.' . $extension; //pasamos como parametro en foto, con el nombre y extensión completo.
		...
		*/
		if ($this->validateUpdate($params)) {
			...
			if (isset($params['imageUri'])) {
				...
				$users = parent::getDB($this->table, $_GET);
				$usuario = $users[0];
				$imagen_antigua = $usuario['imageUri'];
				//echo $imagen_antigua;
